...
export const injectForm = (
  ...
  return class ImmutableForm extends Component {
    constructor(props) {
      ...
      const blankValidation = Object.keys(_validationConfig).reduce(
        (acc, curr) => ({
          ...acc,
          [curr]: null
        }),
      ...
      Observable::merge(
        ...
        this.updateField$::map(([pointer, updater]) => [
          pointer,
          this.state.model.updateIn(pointer, updater)
        ])
      ) // Fire validation
        ::_do(args => validateField$.next(args)) // Update Model
        ::map(([pointer, model]) => ({ model }))
        .subscribe(updateState$);

      const fieldValidated$ = Observable::merge(
        validateField$
      ...
      fieldValidated$
        ...
        ::map(() => {
          const values: Array<boolean | Error> = Object.values(
      ...
      this.submit$
        ::filter(() =>
          Object.values(this.state.validation).every(val => val === true)
        )
        ::map(([props, model]) => [props, model.toJS()])
        ::mergeMap(async args => await onSubmit(...args))
        .subscribe(this.submitted$);

      this.submit$
        ::mergeMap(model =>
          Object.entries(this.state.validation).map(([key, val]) => [
            key,
            val,
            model
          ])
    ...
    async submitForm() {
      const promise = this.submitted$::take(1)::toPromise();
